...
3. Verify the spec and plan exist, the plan has no `- [ ]` task, and `python -m unittest tests/test_greeting.py` passes.
4. Verify the phase names in `<stateDir>/phase-calls.log` are exactly `design`, `plan`, `execute`, and `finish` in that order.
5. Verify `greeting.py` differs from `HEAD`, while `.gitignore`, `.smolpowers.json`, and `tests/test_greeting.py` do not.
