...
3. Write the matching `<specDir>/plans/<date>-override-lifecycle.md`. Include one unchecked task to change only repository-root `greeting.py` so the existing test passes, verify it with `python -m unittest tests/test_greeting.py`, then check the task.
