...
3. Write `<specDir>/specs/<today>-override-lifecycle-design.md` with `Status: Current`. Its required outcome is for repository-root `greeting.py` to make the existing `tests/test_greeting.py` pass by returning exactly `hello from smolpowers`.
