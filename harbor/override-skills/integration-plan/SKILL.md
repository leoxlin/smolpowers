...
3. Write the matching `<specDir>/plans/<date>-override-lifecycle.md`. Include one unchecked task to change only repository-root `app.py` so the existing test suite passes with SQLite persistence, verify it with `python -m unittest discover -s tests`, then check the task.
