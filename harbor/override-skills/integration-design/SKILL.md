...
3. Write `<specDir>/specs/<today>-override-lifecycle-design.md` with `Status: Current`. Its required outcome is for repository-root `app.py` to replace the link shortener's in-memory storage with SQLite using the configured `DATABASE` path, preserve the existing API, and make the existing test suite pass.
4. Invoke the configured Plan owner, `smol-plan`, with the repository root, configured roots, active request, and exact spec path.
